...
class HeartBeatView : AppCompatImageView {
    ...
    fun start() {
        isHeartBeating = true
        animate().scaleXBy(scaleFactor).scaleYBy(scaleFactor).setDuration(duration.toLong()).setListener(scaleUpListener)
    }
    ...
    private val scaleUpListener: Animator.AnimatorListener = object : Animator.AnimatorListener {
        ...
        override fun onAnimationEnd(animation: Animator) {
            ...
            animate().scaleXBy(reductionScaleFactor).scaleYBy(reductionScaleFactor).setDuration(duration.toLong())
    ...
    private val scaleDownListener: Animator.AnimatorListener = object : Animator.AnimatorListener {
        ...
        override fun onAnimationEnd(animation: Animator) {
            if (isHeartBeating) {
                ...
                animate().scaleXBy(scaleFactor).scaleYBy(scaleFactor).setDuration((duration * 2).toLong()).setListener(scaleUpListener)
            }
        }
    ...
    companion object {
        ...
        private const val DEFAULT_SCALE_FACTOR = 0.24f
        private const val DEFAULT_DURATION = 120
