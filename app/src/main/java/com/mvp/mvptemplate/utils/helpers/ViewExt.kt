...
/**
 * Extension method to set View's itemWidth.
 */
...
/**
 * Extension method to resize View with height & itemWidth.
 */
...
fun <T : View> T.click(block: (T) -> Unit) {
    clicks().debounce(350, TimeUnit.MILLISECONDS)
            .subscribeOn(AndroidSchedulers.mainThread())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe { block(this) }
}
...
@SuppressLint("CheckResult")
@Suppress("unchecked_cast")
fun <T : View> T.click(delay: Long, block: (T) -> Unit) {
    clicks().debounce(delay, TimeUnit.MILLISECONDS)
            .subscribeOn(AndroidSchedulers.mainThread())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe { block(this) }
}
